  ...
  <div class="shadow-lg flex flex-wrap">
    ...
    <div class="rounded-lg bg-cover bg-bottom border w-full md:w-1/3 h-64 md:h-auto relative" :style='{ backgroundImage: "url(" + article.image + ")", }'>
      <div class="absolute text-xl">
        <svg @click="unbookmark" class="fill-current text-red-500 hover:text-yellow-400 ml-2 mt-2 cursor-pointer" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path d="M18 1l-6 4-6-4-6 5v7l12 10 12-10v-7z"/></svg>
      </div>
    </div>
    <!-- ./Card image -->
      ...
      <div class="rounded-lg h-full mx-auto px-6 md:px-0 md:pt-6 md:-ml-6 relative">
        ...
        <div class="rounded-lg bg-white lg:h-full p-6 -mt-6 md:mt-0 relative mb-4 md:mb-0 flex flex-wrap md:flex-wrap items-center">
          ...
          <div class="w-full lg:w-1/5 mt-6 lg:mt-0 lg:px-4 text-center md:text-left">
            <button @click="toTimeline(article._id)" class="bg-white rounded-lg hover:bg-grey-darker hover:text-white border border-solid border-grey w-1/3 lg:w-full py-2">Detail</button>
          </div>
          <!-- ./Call to action button -->
        </div>
        <!-- ./Card body - inner wrapper -->
...
export default {
  ...
  methods: {
    unbookmark() {
      this.$emit('unbookmark', this.article._id)
    },
    toTimeline(id) {
